...
describe('GetAppointmentFeedbackUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<GetAppointmentFeedbackUseCase>(
      GetAppointmentFeedbackUseCase,
    ); // Get the target use case
    ...
    it('should successfully process event, generate feedback request message, log, and emit to queue', async () => {
      ...
      expect(mockGenerativeAI.generateContent).toHaveBeenCalledWith(
        expect.stringContaining(
          'how much they like the consultation from 0 to 5',
        ), // Check for feedback intent in prompt
      );
